...
def ARPIP_to_indelMaP_format(path_to_ARPIP_ASR, path_to_alignment_file, path_to_ARPIP_nwk, path_to_ARPIP_ml_indelpoints, path_to_ouput_internal_leaves_msa, path_to_output_internal_leaves_events):
    ...
    ml_indel = pd.read_csv(ml_indelpoints,sep=';',names=n_nodes, engine='python')
...
def compare_trees(tree1, tree2):
    ...
    for node1, node2 in zip(t1.traverse(), t2.traverse()):
        if abs(node1.dist - node2.dist) > 0:  # adjust the epsilon value as needed
            differences.append((node1.name, node1.dist, node2.dist))
        ...
        if MSA_tool == 'ProPIP' or (MSA_tool == 'prank' and gene == 'env'):
            for subset in glob.glob('/cfs/earth/scratch/seppemic/TM/results/MSA/'+gene+'/HIV-1_'+gene+'_MSA_'+MSA_tool+'_*.fasta'):
                ...
                for split in range(1,split_num + 1):
                    ...
                    # Check if branch length were modified by ArPIP
                    differences = compare_trees('/cfs/earth/scratch/seppemic/TM/results/AR/'+gene+'/'+MSA_tool+'_ArPIP/HIV-1_'+gene+'_MSA_'+MSA_tool+'_AR_ArPIP_'+subset_num+'.'+str(split)+'.nwk', '/cfs/earth/scratch/seppemic/TM/data/'+gene+'_subset/HIV-1_'+gene+'_timetree_subset_'+subset_num+'.'+str(split)+'.nwk')
                    if differences:
                        print("Differing branch lengths:")
                        for node_name, dist1, dist2 in differences:
                            print(f"Node {node_name}: {dist1} vs {dist2}")
                    else:
                        print("The trees have identical branch lengths.")
            ...
            for split in range(1,split_num + 1):
                ...
                # Check if branch length were modified by ArPIP
                differences = compare_trees('/cfs/earth/scratch/seppemic/TM/results/AR/'+gene+'/'+MSA_tool+'_ArPIP/HIV-1_'+gene+'_MSA_'+MSA_tool+'_AR_ArPIP.'+str(split)+'.nwk','/cfs/earth/scratch/seppemic/TM/data/'+gene+'/HIV-1_'+gene+'_DNA_mafft_alignment.'+str(split)+'.nwk')
                if differences:
                    print("Differing branch lengths:")
                    for node_name, dist1, dist2 in differences:
                        print(f"Node {node_name}: {dist1} vs {dist2}")
