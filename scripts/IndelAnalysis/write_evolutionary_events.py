...
def write_evolutionary_events(path_to_events_all, path_to_nwk, sub_rate, protein):
    ...
    for node in tree.traverse('preorder'):
        if not node.is_root() and not re.search(r'K03455', node.name):
            ...
            previous_PNGS = ['',0,0] # status, site, position of match extracted sequence
            ...
            for site in range(len(node.sequence)):
                ...
                if char.islower() and char_up == '*':
                    ...
                    PNGS_match = check_if_PNGS(node.sequence,node.up.sequence,site)
                    if PNGS_match[0] != previous_PNGS[0] and site != previous_PNGS[1] + 1 and PNGS_match[1] != int(previous_PNGS[2]) - 1:
                        PNGS_status = PNGS_match[0]
                        previous_PNGS = [PNGS_match[0], site, PNGS_match[1]]
                    else:
                        PNGS_status = 'NA'
                        previous_PNGS = [PNGS_match[0], site, PNGS_match[1]]
