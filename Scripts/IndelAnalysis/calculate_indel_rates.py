...
def calculate_indel_rates(events_df, path_to_events_all, path_to_nwk, protein):
    ...
    for node in events_df['child'].unique():
        ...
        tMRCA =  sub_df['tMRCA'].unique()[0] #previously 'time in years'

        if min_bl >= 1.0:
            min_bl = 0.0

        if tMRCA > (min_bl / sub_rates_dic[protein]):
            seq = tree.search_nodes(name=node)[0].sequence

            # modified on 22.11.21 to take the length of the parent sequence for rates computation
            ...
            for j in range(len(regs)): # 0 to number of regions -1
                ...
                for site in range(start_reg,end_reg+1):
                    if parent_seq[site] not in ['*','-']:       # modified on 22.11.21 to take the length of the parent sequence for rates computation
                        length_reg += 1
